<template>
<div ref="node" :style="flowNodeContainer" @mouseenter="showDelete" @mouseleave="hideDelete" @mouseup="changeNodeSite" :left="node.left" :top="node.top" disabled :mask="false">
  <div :class="b('node',{'active':active===node.id})">
    <div :class="b('node-header')">
      <i class="el-icon-rank" :class="b('node-drag')"></i>
      <slot name="header" :node="node">
      </slot>
    </div>
    ...
    flowNodeContainer: {
      get() {
        return {
          position: 'absolute',
          width: '200px',
          top: setPx(this.node.top),
          left: setPx(this.node.left),
