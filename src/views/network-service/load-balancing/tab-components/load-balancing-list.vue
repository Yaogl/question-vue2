  ...
  <div class="load-balancing-list-container">
    ...
    <el-card shadow="never" class="table-box">
      ...
      <el-table
        ...
        <el-table-column label="状态" prop="name" v-if="showList.includes('3')">
          <template slot-scope="scope">
            {{ scope.row.status === 'ACTIVE' ? '可用' : '不可用' }}
          </template>
        </el-table-column>
        <el-table-column label="监听器个数" prop="subnet_num" v-if="showList.includes('2')" />
  ...
  data() {
    return {
      ...
      headerList: [
        ...
        { label: '监听器个数', value: '2', key: 'availability_zone' },
        { label: '状态', value: '3', key: 'flavor' },
        { label: 'VPC网络', value: '4', key: 'subnet_num' },
        { label: 'IP地址', value: '5', key: 'mtu' },
        { label: '计费方式', value: '6', key: 'project_name' },
