<template lang="html">
  <div class="vpn-connect-list-container">
    <el-row>
      <el-col :span="12">
        <el-button type="ghost" v-if="authBtns.VPN_CONNECT_REFRESH_BTN">
    ...
    <el-card shadow="never" class="table-box">
      ...
      <el-table
        ...
        <el-table-column label="状态" prop="mtu" v-if="showList.includes('2')">
          <template slot-scope="scope">
            {{ scope.row.status === 'ACTIVE' ? '连接中' : '未连接' }}
          </template>
        </el-table-column>
        <el-table-column label="VPN网关" prop="name" v-if="showList.includes('3')" />

        <el-table-column label="本端子网" v-if="showList.includes('4')" />
        <el-table-column label="本端网关IP" v-if="showList.includes('5')" />
        <el-table-column label="创建时间" prop="created_at" v-if="showList.includes('6')" />
        <el-table-column label="操作" width="160">
          <template lang="html" slot-scope="scope">
            <el-button type="text">策略详情</el-button>
  ...
  data() {
    return {
      ...
      headerList: [
        ...
        { label: 'VPN网关', value: '3', key: 'availability_zone' },
        { label: '本端子网', value: '4', key: 'project_name' },
        { label: '本端网关IP', value: '5', key: 'created_at' },
