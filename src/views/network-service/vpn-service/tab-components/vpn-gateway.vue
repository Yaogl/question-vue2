      ...
      <el-table
        ...
        <el-table-column label="状态" prop="mtu" v-if="showList.includes('2')">
          <template slot-scope="scope">
            {{ scope.row.status === 'ACTIVE' ? '运行中' : '关闭' }}
          </template>
        </el-table-column>
        <el-table-column label="类型" prop="" v-if="showList.includes('3')">
          <template slot-scope="scope">
            IPsec
          </template>
        </el-table-column>
        <el-table-column label="VPC网络" prop="name"  v-if="showList.includes('4')" />
        <el-table-column label="网关IP" prop="ip" v-if="showList.includes('5')" />
        <el-table-column label="计费方式" prop="cashtype" v-if="showList.includes('6')" />
  ...
  data() {
    return {
      ...
      headerList: [
        ...
        { label: '类型', value: '3', key: 'availability_zone' },
        { label: 'VPC网络', value: '4', key: 'flavor' },
        { label: '网关IP', value: '5', key: 'project_name' },
        { label: '计费方式', value: '6', key: 'created_at' },
